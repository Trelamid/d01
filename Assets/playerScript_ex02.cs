...
public class playerScript_ex02 : MonoBehaviour
{
    ...
    void Move()
    {
        if(Input.GetKey(KeyCode.RightArrow))
            _rigidbody2D.AddForce(Vector2.right  *_speed * _rigidbody2D.mass * Time.deltaTime * 100);
        if(Input.GetKey(KeyCode.LeftArrow))
            _rigidbody2D.AddForce(Vector2.left *_speed * _rigidbody2D.mass * Time.deltaTime * 100);
